    ...
    private void Update()
    {
        if(!stunned)
        {
            ...
            else if(!IsWork)
            {
                ...
                {
                    WorkPlace newPlace = _serchObject.SearchWorkPlace(this.transform.parent.gameObject);
                    if(newPlace != null)
                    {
                        newPlace.SetWorker(this);
                        _movement.ChangeTarget(newPlace.transform.GetChild(0).transform.position);
                    } 
                }
            }
        }
